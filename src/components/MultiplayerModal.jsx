...
function DiscordSettings({ mp }) {
  ...
  const saveUrl = (v) => { setUrl(v); setWebhook(v); setTest(null); };
  const toggle = (k) => { const next = { ...opts, [k]: !opts[k] }; setLocalOpts(next); setOpts(next); };
  const runTest = async () => { setTest('pending'); setTest((await testWebhook(url)) ? 'ok' : 'bad'); };

  const valid = isValidWebhook(url);

  return (
    <details className="mp-discord">
      <summary><MessagesSquare size={15} /> {t('mp.discord.title')}</summary>
      <div className="mp-discord-body">
        ...
        <div className="mp-discord-opts">
          <label><input type="checkbox" checked={opts.rolls} onChange={() => toggle('rolls')} /> {t('mp.discord.rolls')}</label>
          <label><input type="checkbox" checked={opts.events} onChange={() => toggle('events')} /> {t('mp.discord.events')}</label>
        </div>

        {mp.role === 'gm' && Object.keys(mp.players).length > 0 && valid ? (
          <button type="button" className="btn btn-sm btn-ghost" onClick={() => mp.sendGmCommand(null, { cmd: GM_WEBHOOK, url })}>
      ...
      {!mp.role ? (
        <div className="mp-choice">
          ...
          <Field label={t('mp.joinCode')}>
            <div className="inline-roll">
              <TextInput value={code} onChange={(v) => setCode(v.toUpperCase().slice(0, 4))} placeholder="ABCD" maxLength={4} />
        ...
        <div className="mp-active">
          <div className="mp-room">
            ...
            {(() => {
              const online = mp.role === 'gm' ? mp.roomOnline : mp.connectionState === 'connected';
              return <span className={`mp-dot ${online ? 'on' : 'off'}`}>{online ? t('mp.roomOnline') : t('mp.roomOffline')}</span>;
            })()}
          </div>
          ...
          {mp.role === 'gm' ? (
            <>
              ...
              <p className="hint"><Users size={14} /> {t('mp.playersConnected', { n: Object.keys(mp.players).length })}</p>
