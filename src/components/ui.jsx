...
export function Stepper({ value, onChange, min = -99, max = 999, label }) {
  return (
    <div className="stepper" role="group" aria-label={label}>
      <button type="button" onClick={() => onChange(Math.max(min, (value || 0) - 1))} aria-label="-">−</button>
      <input
        type="number"
      ...
      />
      <button type="button" onClick={() => onChange(Math.min(max, (value || 0) + 1))} aria-label="+">+</button>
    </div>
  );
}
    ...
    <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className={`modal${wide ? ' modal-wide' : ''}`} role="dialog" aria-modal="true" aria-label={title}>
        <div className="modal-head">
          <h2>{title}</h2>
          <button type="button" className="icon-btn" onClick={onClose} aria-label={t('common.close')}><X size={18} /></button>
        </div>
...
export function InfoHint({ text }) {
  ...
  return (
    <span className="info-hint">
      <button ref={btnRef} type="button" className={`info-hint-btn${pos ? ' is-open' : ''}`} aria-label={t('header.help')} aria-expanded={!!pos} onClick={toggle}>
        &dagger;
      </button>
      {pos
        ? createPortal(
          <span
            ...
            role="tooltip"
            style={{ position: 'fixed', top: pos.top, left: pos.left, width: pos.width, '--arrow-x': `${pos.arrow}px` }}
          >
            {text}
          </span>,
          document.body,
