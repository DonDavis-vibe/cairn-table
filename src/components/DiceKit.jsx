...
// Wuerfel-Optik: SVG-Glyphen und eine Ergebnis-Buehne, in der der letzte Wurf
// als echter (CSS-)3D-Wuerfel "taumelt" und dann auf seinem Ergebnis landet.
// Portiert aus dem Pips-&-Paws-Wuerfelkit (gleiche Werkzeugfamilie), an das
// Feldjournal-Thema angepasst (siehe die3d-* / dice-stage-* in index.css).
...
export function DieGlyph({ sides, face }) {
  if (sides === 6) return <D6Face face={face} />;
  return <PolyDie sides={sides} />;
}

// Echter 3D-Wuerfel (CSS-Transform, 6 Seiten mit Pips) fuer den Schicksalswuerfel
// (W6). Jede Seite steht per rotateX/Y + translateZ fest im Wuerfelraum; um Seite
// N nach vorn zu drehen, dreht sich der GANZE Wuerfel um die Umkehrung von Seite
// N's eigener Drehung — bei reinen 90/180-Grad-Drehungen um eine Achse ist das
// einfach die Negation (siehe CUBE_SHOW).
...
// --- Generische Vielflaechner (W4/W8/W10/W12), fuer den Schadenswuerfel ---
// Der Wuerfel (oben) hat nur rechte Winkel zwischen seinen Seiten, das geht mit
// einfachem rotateX/rotateY. Die anderen Formen nicht — hier reicht nur eine
// echte Achse-Winkel-Drehung (kuerzester Drehweg zwischen zwei Normalenvektoren),
// sonst identisches Prinzip: jede Seite steht per fester Drehung + translateZ im
// Wuerfelraum, der ganze Koerper dreht sich um die Umkehrung der Ziel-Seiten-
// Drehung, damit die Seite nach vorn zeigt.
...
const FORWARD = [0, 0, 1];
const PHI = (1 + Math.sqrt(5)) / 2; // Goldener Schnitt, fuers Dodekaeder (W12)
...
// Dieselben Eckpunkte wie die `.die3d-face--<shape>` clip-paths in index.css,
...
// beides im selben Uebergang (rotate3d nimmt dabei den kuerzesten Drehweg direkt
...
// Ergebnis-Buehne fuer den letzten Wurf. `result`: { id, label, value, max, die?, parts?, verdict?, tone? }
//   die: Seitenzahl fuer einen ECHTEN 3D-Wuerfel (6 -> Wuerfel, 4/8/10/12 -> Vielflaechner).
//        Fehlt sie (z.B. beim W20-Rettungswurf oder der 2W6-Reaktion), zeigt die
//        Buehne nur die grosse Zahl plus ggf. Einzelwuerfe als Chips.
//   tone: 'ok' | 'bad'
export function DiceStage({ result, idleIcon, idleText }) {
  ...
  if (!result) {
    return (
      <div className="dice-stage stage-idle">
  ...
  const isPlain = !isCube && !isPoly;

  return (
    <div className={`dice-stage stage-live${tone ? ` stage-${tone}` : ''}`}>
      {isCube ? (
        <Die3D value={result.value} rollId={result.id} size={cubeSize} />
      ) : isPoly ? (
        <PolyDie3D sides={result.die} value={result.value} rollId={result.id} size={cubeSize} />
      ) : (
        // Kein echter 3D-Wuerfel dafuer (W20-Rettungswurf, 2W6-Reaktion) — die
        // Zahl bekommt trotzdem ein Zuhause statt einfach in der Luft zu
        // schweben: ein Medaillon, wie die Wachssiegel bei den Attributen.
        <span className="dice-medallion">
      ...
      {!isPlain ? (
        <span className={`dice-stage-value dice-stage-value--sub${rolling ? ' is-rolling' : ''}`} key={`v2-${result.id}`}>
          {shown}
        </span>
      ) : null}
